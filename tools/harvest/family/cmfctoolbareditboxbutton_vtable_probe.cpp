// Harvest the real mfc140u CMFCToolBarEditBoxButton vtable SLOT INDEX of each exported
...
// the base CMFCToolBarButton slots (0-52); new virtuals declared in CMFCToolBarEditBoxButton
// append after slot 52 in SDK-header (afxtoolbareditboxbutton.h) declaration order.
...
#include <cstdio>
#include <cstdint>
#include "cmfctoolbareditboxbutton_virt_ords.h"
int main(){
  if(!AfxWinInit(::GetModuleHandle(NULL),NULL,::GetCommandLine(),0))return 1;
  HMODULE m = GetModuleHandleW(L"mfc140u.dll");
  CMFCToolBarEditBoxButton btn;
  uintptr_t* vptr = *(uintptr_t**)&btn;
  const int N = 57;   // exact slot count (cmfctoolbareditboxbutton_vtable.json); don't over-read

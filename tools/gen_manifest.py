...
Categories:
  real       – Symbol: comment in non-fallback source matching a valid export
...
# Generated ABI fallbacks are useful for keeping the DLL linkable while a
# symbol has no implementation, but they are not implementation progress.
# Detect them from generator-owned headers rather than a filename so copying or
# renaming generated output under phase4/src cannot make coverage report it as
# real code.
GENERATED_FALLBACK_MARKER_SETS = (
    (
...
def is_generated_fallback_source(source_path):
    """Return True when *source_path* is generated default-return fallback code."""
    with open(source_path, encoding='utf-8', errors='replace') as source:
        header = source.read(4096)
    return any(all(marker in header for marker in markers)
               for markers in GENERATED_FALLBACK_MARKER_SETS)
...
def collect_symbol_comments(src_dir):
    ...
    for fpath in iter_sources(src_dir):
        if is_generated_fallback_source(fpath):
            continue

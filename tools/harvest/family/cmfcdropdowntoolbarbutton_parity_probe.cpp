// Parity scorecard for CMFCDropDownToolbarButton: grades OUR built openmfc.dll against the
// harvested real-mfc140u ground truth (cmfcdropdowntoolbarbutton_vtable.json + _meta_probe).
...
// through a vtable it hasn't first proven is the installed MSVC one, so it won't crash on the
// current mingw-stub object).
//
...
//            -o ddparity.exe cmfcdropdowntoolbarbutton_parity_probe.cpp
//   run:   cp build-phase4/openmfc.dll . && WINEDEBUG=-all wine ddparity.exe openmfc.dll
//
// GROUND TRUTH (harvested real mfc140u 14.51.36231):
//   sizeof=176, RTTI schema=0x80000001 (VERSIONABLE_SCHEMA|1), CreateObject non-null
//   (DECLARE_SERIAL factory), vtable=53 slots (no new virtuals; all 12 exported virtuals are
//   overrides of base CMFCToolBarButton slots), IsDropDown(default)=0.
#include <windows.h>
#include <cstdio>
#include <cstring>
...
int main(int argc, char** argv){
  ...
  printf("== CMFCDropDownToolbarButton parity vs real mfc140u (harvested golden) ==\n");
  printf("-- Group A: class metadata --\n");

  Ctor0 c0 = (Ctor0)GetProcAddress(h, "??0CMFCDropDownToolbarButton@@QEAA@XZ");
  grade("ctor ??0...@XZ exported", c0!=0, 0);

  GetRC_fn getrc = (GetRC_fn)GetProcAddress(h, "?GetRuntimeClass@CMFCDropDownToolbarButton@@UEBAPEAUCRuntimeClass@@XZ");
  int rc_obj=-1; unsigned rc_schema=0;
  if(getrc){ void* rc=getrc(0); if(rc){ rc_obj=*(int*)((char*)rc+8); rc_schema=*(unsigned*)((char*)rc+12); } }
  char nb[64];
  snprintf(nb,sizeof nb,"got objsize=%d (exp 176)", rc_obj);
  grade("RTTI objsize == 176", rc_obj==176, nb);
  snprintf(nb,sizeof nb,"got schema=0x%X (exp 0x80000001)", rc_schema);
  grade("RTTI schema == 0x80000001", rc_schema==0x80000001, nb);

  CreateObj_fn co = (CreateObj_fn)GetProcAddress(h, "?CreateObject@CMFCDropDownToolbarButton@@SAPEAVCObject@@XZ");
  ...
  printf("-- Group B: MSVC 53-slot vtable --\n");
  VtblAddr_fn vaddr = (VtblAddr_fn)GetProcAddress(h, "OpenMFC_DropDownToolbarButtonVtableAddr");
  grade("OpenMFC_DropDownToolbarButtonVtableAddr exported", vaddr!=0, vaddr?"":"vtable not installed yet");

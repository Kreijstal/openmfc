// Parity scorecard for CMFCToolBarEditBoxButton: grades OUR built openmfc.dll against
// the harvested real-mfc140u ground truth (cmfctoolbareditboxbutton_vtable.json +
...
//            -o editparity.exe cmfctoolbareditboxbutton_parity_probe.cpp
//   run:   cp build-phase4/openmfc.dll . && WINEDEBUG=-all wine editparity.exe openmfc.dll
//
// GROUND TRUTH (harvested real mfc140u 14.51.36231):
//   sizeof=176, RTTI schema=0x1, CreateObject non-null (DECLARE_SERIAL factory),
//   vtable=57 slots.
#include <windows.h>
#include <cstdio>
#include <cstring>
...
int main(int argc, char** argv){
  ...
  printf("== CMFCToolBarEditBoxButton parity vs real mfc140u (harvested golden) ==\n");
  printf("-- Group A: class metadata --\n");

  Ctor0 c0 = (Ctor0)GetProcAddress(h, "??0CMFCToolBarEditBoxButton@@QEAA@XZ");
  grade("ctor ??0...@XZ exported", c0!=0, 0);

  GetRC_fn getrc = (GetRC_fn)GetProcAddress(h, "?GetRuntimeClass@CMFCToolBarEditBoxButton@@UEBAPEAUCRuntimeClass@@XZ");
  ...
  CreateObj_fn co = (CreateObj_fn)GetProcAddress(h, "?CreateObject@CMFCToolBarEditBoxButton@@SAPEAVCObject@@XZ");
  ...
  printf("-- Group B: MSVC 57-slot vtable (increment-3) --\n");
  VtblAddr_fn vaddr = (VtblAddr_fn)GetProcAddress(h, "OpenMFC_ToolBarEditBoxButtonVtableAddr");
  grade("OpenMFC_ToolBarEditBoxButtonVtableAddr exported", vaddr!=0, vaddr?"":"vtable not installed yet");

...
template<typename Wrapper, typename State>
const State* FindCollectionState(const Wrapper* self) {
    auto& states = GetCollectionStates<Wrapper, State>();
    auto it = states.find(self);
    return it == states.end() ? nullptr : &it->second;
}
...
const value_type& class_name::operator[](arg_key_type key) const { return const_cast<class_name*>(this)->operator[](key); } \

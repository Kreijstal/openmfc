...
struct CColorBtn {
    ...
    CPalHolder* m_pPaletteObj;   // 0xbc0: palette holder (RebuildPalette/OnDraw)
};
    ...
    void* pThis, const wchar_t* lpszLabel, unsigned long clr, int /*bShowBorder*/)
{
    ...
    AssignCStr(&s->m_strAutoLabel, lpszLabel ? lpszLabel : L"");  // 0xbb0
    ...
    void* pThis, const wchar_t* lpszLabel, int bAuto, int /*bShowBorder*/)
{
    if (pThis == nullptr) return;
    CColorBtn* s = reinterpret_cast<CColorBtn*>(pThis);
    AssignCStr(&s->m_strOtherLabel, lpszLabel ? lpszLabel : L"");  // 0xba0
    ...
    void* pThis, void* pPalette)
{
    ...
    CPalHolder* old = s->m_pPaletteObj;
    if (old != nullptr && old->vtable == nullptr) {
        if (old->m_hPal != nullptr) ::DeleteObject(old->m_hPal);
        std::free(old);
    }

    CPalHolder* holder = static_cast<CPalHolder*>(std::calloc(1, sizeof(CPalHolder)));
    ...
    void* pThis, const wchar_t* lpszLabel, void* /*lstColors*/)
{
    ...
    AssignCStr(&s->m_strDocLabel, lpszLabel ? lpszLabel : L"");  // 0xba8
}
